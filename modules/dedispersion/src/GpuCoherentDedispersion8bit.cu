...
#include <cufftw.h>
#include <cstring>
#include "../GpuCoherentDedispersion8bit.h"
...
GpuCoherentDedispersion8bit::GpuCoherentDedispersion8bit(int nchans, int fft_len, float dm)
    :   _nchans(nchans)
    ...
    ,   _cpu_temp(fft_len*nchans)
{
    ...
    CUDA_CHECK(cudaMalloc((void**)&_gpu_data_in, fft_len*nchans*sizeof(char)*2));
...
GpuCoherentDedispersion8bit::~GpuCoherentDedispersion8bit()
{
//    (_cpu_fwd);
//    fftwf_free(_cpu_inv);
    cufftDestroy(_planf);
...
int GpuCoherentDedispersion8bit::dedisperse(char* data_in, std::vector<float>& data_out)
{
    unsigned block_size = _gpu_fft_len*_nchans;
    std::cout<<(int)data_in[10]<<" "<<(int)data_in[100]<<"\n";
    cudaMemcpy(&_gpu_data_in[0], reinterpret_cast<char*>(data_in), sizeof(int8_t)*block_size, cudaMemcpyHostToDevice);
    ...
    typecast_in<<<block_size/2048, 1024>>>(&_gpu_in[0], reinterpret_cast<char*>(&_gpu_data_in[0]), block_size/2);
